...
india.full.data <- read.csv('/Users/roseschneider/Documents/Projects/xculture/India/Data/India_Data.csv')%>%
  filter(SID != "CopyPasteMe",
         Language != "ChangeMe")%>%
  droplevels()%>%
  dplyr::select(-Response_single, -Response_double, -Response_mismatch, -Mem_check_1_single, 
...
india.hc %<>%
  filter(Exclude != 1, 
         Exclude != "HELP", 
         Language != "")%>%
  dplyr::select(-IHC_single, -FHC_single)%>%
  dplyr::rename(IHC = IHC_final, 
                FHC = FHC_final)%>%
...
##Gujarati Unit Task (interim) within-language results: FHC and Productivity emerge as best predictor of performance on Unit Task
...
#Unit Task: Cross linguistic models - Warning, Convergence issues! 
##Build the models
```{r}
#base
...
mtable.sf.cross.large <- mtable('Highest contig. alone' = sf.cross.within.plus1,
            'Productive + Highest Contig.' = sf.cross.within.plus2,
            'FHC + Highest Contig.' = sf.cross.within.plus2.prod,
            'Highest contig. + Prod. + FHC' = sf.cross.within.plus3,
            summary.stats = c('R-squared','F','p','N'))
...
#Cross-linguistic analyses, Unit Task (India)
...
---
#Cross-linguistic analysies, NN Task (India)
