...
#manually add child who broke productivity code: CH-VC-1, IHC 20, FHC 56
...
productive <- right_join(productive, lang, by =  "SID")
...
all.data %>%
  filter(Task == "SF") %>%
  filter(!is.na(Productive))%>%
  group_by(SID, IHC, Language, Productive)%>%
  summarise(mean_unit = mean(Correct, na.rm = TRUE))%>%
  ggplot(aes(x = IHC, y = mean_unit, colour = Language, group = Language)) +
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
...
all.data %>%
  filter(Task == "SF") %>%
  filter(!is.na(Productive))%>%
  group_by(SID, FHC, Language, Productive)%>%
  summarise(mean_unit = mean(Correct, na.rm = TRUE))%>%
  ggplot(aes(x = FHC, y = mean_unit, colour = Language, group = Language)) +
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
...
all.data %>%
  ...
  group_by(highest_contig, Language)%>%
  summarise(mean = mean(Correct, na.rm = TRUE), 
            n = n(), 
            sd = sd(Correct, na.rm = TRUE), 
            se = sd/sqrt(n)) %>%
...
sf.hk.within.plus3 <- glmer(Correct ~ ihc.c + fhc.c + highest_contig.c + count_range + age.c + 
                              (1|SID), family = "binomial", data = sf.hk.within)
anova(sf.hk.within.plus3, sf.hk.within.plus2, sf.hk.within.plus1, test = 'LRT') 
summary(sf.hk.within.plus3)
```
