...
hk.full.data <- read.csv('Data/HK_data.csv')%>%
  ...
  mutate(Age = as.numeric(as.character(Age)), 
         ...
         Mem_check_1 = as.integer(Mem_check_1), 
         Mem_check_2 = as.integer(Mem_check_2))
...
###Plotting distance between Initial and Final Highest Counts (~Pierina's graph)
####Cantonese
```{r}
ggplot(subset(unique.hc.data, dataset == "HK"), aes(x = SID, y = highest_count)) + 
  ...
  labs(title="Distance between initial and final highest counts - Cantonese",
       x = "Each line = individual kids",
       y="Highest Count by Count Type") +
  theme_bw(base_size = 7) + 
  theme(legend.position="bottom", 
        axis.text.x = element_text(angle = 90, hjust = 1)) +
  facet_grid(~productive, scales = "free_x")
```

####Slovenian
```{r}

ggplot(subset(unique.hc.data, dataset == "SLO"), aes(x = SID, y = highest_count)) + 
  facet_grid(rows = vars(productive)) +
  geom_line() + 
  geom_point(aes(shape = IHC_FHC, colour = IHC_FHC), 
             size = 2, stroke = 1.5) +
