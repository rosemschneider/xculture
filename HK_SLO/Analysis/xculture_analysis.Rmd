...
TO-DO RMS: This is a mess, fix it up
```{r}
sf.ms <- all.data %>%
  filter(!is.na(Productive))%>%
  filter(Task == "SF")%>%
  ...
  group_by(Task_item, Language)%>%
  summarise(mean = mean(as.numeric(as.character(Correct), na.rm=TRUE)),
            sd = sd(as.numeric(as.character(Correct), na.rm=TRUE)))%>%
  mutate(se = sd/(sqrt(length(unique(subset(all.data, Task == "SF")$SID)))), 
         upper = mean+se, 
         lower = mean-se)

spline_int <- as.data.frame(spline(sf.ms$Task_item, sf.ms$mean))
  
  ggplot(sf.ms, aes(x = Task_item, y = mean, fill=Language)) +
    geom_point(position = position_dodge(width = .95)) + 
    geom_errorbar(aes(ymin=lower, ymax = upper), width=0.2, size=1) + 
    stat_smooth(aes(y=mean, x=Task_item), method = lm, formula = y ~ poly(x, 23), se = FALSE)
    scale_x_continuous(breaks = c(5, 7, 16, 24, 52, 71, 105, 107, 116, 224, 252, 271)) + 
    ylab("Mean correct")
    xlab('') + 
    facet_grid(~Productive)+
    theme_bw()
  ...
  facet_grid(~Productive)+
  theme_bw() + 
  ggtitle("Performance on Unit Task by language, productivity, within/outside IHC") +
  theme(text = element_text(size = 12)) +
  ylim(0, 1.0) +
    scale_x_continuous(breaks = c(5, 7, 16, 24, 52, 71, 105, 107, 116, 224, 252, 271)) +
...
all.data %>%
  ...
  group_by(SID, Task, Productive, Language)%>%
  summarise(mean = mean(as.numeric(as.character(Correct), na.rm=TRUE)),
            sd = sd(as.numeric(as.character(Correct), na.rm=TRUE))) %>%
  ggplot(aes(x = Task, y = mean, fill=Language)) +
  stat_summary(fun.y = mean, position = position_dodge(width = .95), 
                      geom="bar", colour = "black") +
  stat_summary(fun.data = mean_se, geom="errorbar", 
               position = position_dodge(width=0.90), width = 0.2)+
  ...
  ggtitle("Performance on Unit and NN Tasks by language, productivity") +
  theme(text = element_text(size = 12)) +
  ylim(0, 1.0) +
  scale_fill_brewer(palette = "Dark2")
```
