...
hc.df %<>%
  ...
  filter(SID != "CH-VC-1", 
         ...
         SID != "CH-MV-14", 
         SID != 'GV-21-SB', 
         SID != 'RT09')#excluding these kids because they break my productivity code
...
mean_prompts <- error.freq %>%
  group_by(SID, Language, Productive)%>%
  summarise(n = n())%>%
  group_by(Language, Productive)%>%
  summarise(mean_prompts = mean(n), 
            sd_prompts = sd(n))
...
error.freq %>% 
  ...
  mutate(Language = ifelse(Language == "English (US)", "English", as.character(Language)))%>%
  group_by(Language, Productive, Error_type)%>%
  summarise(n = n()) %>%
  ggplot(aes(x = Error_type, y = n, fill = Productive)) + 
  ...
  labs(x = "Error type", y = "Number of errors")
...
initial_final %>%
  filter(Dataset != "India")%>%
  ggplot(aes(x = reorder(LadlabID, hc, FUN=min), y = hc)) + 
