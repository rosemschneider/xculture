...
#   print("ERROR: Check correct coding")
# }
```
...
all.data %>%
  ...
  mutate(prop = n/31)%>%
  filter(prop < .8)
##Decision - Indian English, excluding kids who do not have NN, keeping kids with low number of trials due to low SF
...
all.data %>%
  ...
  group_by(Productive, Language, highest_contig)%>%
  summarise(n = n()) %>%
  ggplot(aes(x = highest_contig, y = n, fill = Productive)) + 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  labs(y = "Count", x = "Highest Contiguous NN")
...
hcnn.hk <- all.data %>%
  filter(Language == "English (US)" |
           Language == "Cantonese" |
           Language == "Slovenian")%>%
  mutate(Language = factor(Language, levels = c("English (US)", "Cantonese", "Slovenian")))%>%
...
all.data %>%
  ...
  distinct(SID, highest_contig, Language, Age, sum_wppsi, Productive)%>%
  group_by(Language, Productive)%>%
  summarise(mean = round(mean(highest_contig, na.rm = TRUE), 2), 
            sd = round(sd(highest_contig, na.rm = TRUE), 2), 
            median = round(mean(highest_contig, na.rm = TRUE, 2)))
```
...
###Base v. FHC - Sig, Chisq(1) = 11.65, p = 0.0006, AIC = 1614.6
```{r}
...
#Explore - mean NN v. IHC
sf.hk.within.plus1 <- glmer(Correct ~ ihc.c + count_range + starting_num.c + age.c + (1|SID), 
                             family = "binomial", 
                           data = sf.hk.within)

sf.hk.within.plus.nn <- glmer(Correct ~ mean.nn + ihc.c + count_range + starting_num.c + age.c + (1|SID), 
                             family = "binomial", 
                           data = sf.hk.within)

anova(sf.hk.within.plus1, sf.hk.within.plus.nn, test = 'LRT')

summary(sf.hk.within.plus1)
summary(sf.hk.within.plus.nn)

##What about productivity gradient?
sf.hk.within.plus.gain <- glmer(Correct ~ prod.gradient + ihc.c + count_range + starting_num.c + age.c + (1|SID), 
                             family = "binomial", 
                           data = sf.hk.within)
anova(sf.hk.within.plus1, sf.hk.within.plus.gain, test = 'LRT')
```
...
##Model Comparisons
###Base v. Productivity - sig, Chis(1) = 11.71, p = .0006, AIC = 1385.7
```{r}
anova(sf.slo.within.base, sf.slo.within.model1, test = 'LRT')
```

 
###Base v. FHC - Sig, Chisq(1) = 15.19, p< .0001, AIC = 1317.5
```{r}
...
##Model comparisons
###Base v. Productivity - Sig, Chisq(1) = 26.13, p < .0001, AIC = 839.57
```{r}
anova(wcn.slo.within.base, wcn.slo.within.model1, test = 'LRT')
```

###Base v. FHC - Sig, Chisq(1) = 46.24, p < .0001, AIC = 819.5
```{r}
anova(wcn.slo.within.base, wcn.slo.within.model2, test = 'LRT')
```

###Base v. IHC - Sig, Chisq(1) = 16.07 p < .0001, AIC = 849.6
```{r}
...
###Base v. Highest contig. - Sig, Chisq(1) = 25.824, p < .0001, AIC = 4321.3
```{r}
...
##Model comparisons
###Base v. Productivity - Sig, Chisq(1) = 17.02, p < .0001, AIC = 2856.8
```{r}
anova(wcn.cross.base, wcn.cross.model1, test = 'LRT')
```

###Base v. FHC - Sig, Chisq(1) = 39.89, p < .0001, AIC = 2925.1
```{r}
