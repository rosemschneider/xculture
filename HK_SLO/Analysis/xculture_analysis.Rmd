...
#Exploratory - productivity gain - Marginal, Chisq(1) = 3.87, p = .05, AIC = 1622.5
```{r}
...
##Model comparisons
###Base v. Productivity - NS, Chisq(1) = 1.22, p = .27
```{r}
anova(sf.us.within.base, sf.us.within.model1, test = 'LRT')
```

###Base v. FHC - Sig, Chisq(1) = 18.82, p < .0001, AIC = 1527.9
```{r}
...
###Base v. Highest Contig. - Sig, Chisq(1) = 19.6, p < .0001, AIC = 1527.1
```{r}
anova(sf.us.within.base, sf.us.within.model4, test = 'LRT')
```

###Post-hoc: Prod.gradient - Sig, Chisq(1) = 4.3153, p = .04, AIC = 1517.3
```{r}
...
###Exploratory - only looking at items not produced in IHC
```{r}
sf.us.within.lim <- sf.us.within %>%
  filter(Task_item >= 224)

#base
sf.us.within.base <- glmer(Correct ~ age.c + (1|SID) + (1|Task_item), family = "binomial", 
                           data = sf.us.within.lim)
#productivity
sf.us.within.model1 <- glmer(Correct ~ Productive + age.c + (1|SID)+ (1|Task_item), family = "binomial", 
                           data = sf.us.within.lim)
#FHC
sf.us.within.model2 <- glmer(Correct ~ fhc.c + age.c + (1|SID)+ (1|Task_item), family = "binomial", 
                           data = sf.us.within.lim)
#IHC
sf.us.within.model3 <- glmer(Correct ~ ihc.c + age.c + (1|SID)+ (1|Task_item), 
                             family = "binomial", 
                           data = sf.us.within.lim, control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#Highest contiguous
sf.us.within.model4 <- glmer(Correct ~ highest_contig.c + age.c + (1|SID)+ (1|Task_item), 
                             family = "binomial", 
                           data = sf.us.within.lim) 
#Exploratory, Prod. gradient
sf.us.within.model.gain <- glmer(Correct ~ prod.gradient + age.c + (1|SID) + (1|Task_item), 
                             family = "binomial", 
                           data = sf.us.within.lim) #ns
