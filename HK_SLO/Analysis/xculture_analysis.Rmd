...
xdens.slo <- axis_canvas(pmain, axis = "x") + 
  geom_ridgeline(data = slo_initial, aes(x = IHC, y = 0, height=..density.., 
                                        fill = Productive), 
                 stat = 'xdensity', alpha = .5, size = .2, trim = FALSE) + 
  scale_fill_manual(values = productivity.pal)

ydens.slo <- axis_canvas(pmain, axis = "y") + 
...
##Model comparisons
###Base v. Productivity - NS, Chisq(1) = 1.10, p = .29, AIC = 1516.3
```{r}
...
###Base v. FHC - sig, Chisq(1) = 21.5, p < .0001, AIC = 1327.3
```{r}
anova(sf.hindi.within.base, sf.hindi.within.model2, test = 'LRT')
```

###Base v. IHC - Sig, chisq(1) = 28.45, p < .0001, AIC = 1320.4
```{r}
anova(sf.hindi.within.base, sf.hindi.within.model3, test = 'LRT')
```

###Base v. Highest contig. - Sig, Chisq(1) = 28.26, p < .0001, AIC = 1320.5
```{r}
...
###Base v. FHC - Sig, Chisq(1) = 21.56, p < .0001, AIC = 681.18
```{r}
anova(wcn.ind.eng.within.base, wcn.ind.eng.within.model2, test = 'LRT')
```

###Base v. IHC - Sig, Chisq(1) = 19.68, p < .0001, AIC = 683.06
```{r}
...
##Model comparisons
###Base v. Productivity - sig, Chisq(1) = 4.64, p = .03, AIC = 853.39
```{r}
...
#Exploratory - Prod. gradient, NS - p = .43
```{r}
