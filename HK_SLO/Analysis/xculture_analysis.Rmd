...
us.full.data <- read.csv('/Users/roseschneider/Documents/Projects/xculture/HK_SLO/Data/US_data.csv') %>%
  ...
  mutate(Language = ifelse(Language == "English", "English - US", as.character(Language)))
...
india.full.data <- read.csv('/Users/roseschneider/Documents/Projects/xculture/India/Data/India_Data.csv')%>%
  filter(SID != "CopyPasteMe", 
         Language != "ChangeMe")%>%
  droplevels()%>%
  mutate(Language = ifelse(Language == "Hin", "Hindi", as.character(Language)), 
         Language = ifelse(Language == "HIndi", "Hindi", as.character(Language)), 
         Language = ifelse(Language == "Guajrati", "Gujarati", as.character(Language)), 
         Language = ifelse(Language == "Gujarayi", "Gujarati", as.character(Language)), 
         Language = ifelse(Language == "Hindi ", "Hindi", as.character(Language)),  
         Language = ifelse(Language == "Gujarati ", "Gujarati", as.character(Language)), 
         Language = ifelse(Language == "Gujar", "Gujarati", as.character(Language)), 
         Language = ifelse(Language == "Engish", "English", as.character(Language)), 
...
hc.df <- bind_rows(slo.hc, hk.hc, us.hc)
```
...
sf.slo.within.plus2 <- glmer(Correct ~ ihc.c + highest_contig.c + count_range + age.c + (1|SID), 
                             data = sf.slo.within, family = "binomial")
anova(sf.slo.within.plus2, sf.slo.within.plus1, test = 'LRT')
```
...
#English: Within-language models, Unit Task
Build the models
##Model 0 (base)
```{r}
...
sf.us.within.plus2 <- glmer(Correct ~ fhc.c + highest_contig.c + count_range + age.c + (1|SID), 
                             data = sf.us.within, family = "binomial")
anova(sf.us.within.plus2, sf.us.within.plus1, test = 'LRT')
sf.us.within.plus2 <- glmer(Correct ~ ihc.c + highest_contig.c + count_range + age.c + (1|SID), 
                             data = sf.us.within, family = "binomial")
anova(sf.us.within.plus2, sf.us.within.plus1, test = 'LRT')
```
...
anova(wcn.us.within.plus2, wcn.us.within.plus1, test = 'LRT') #Productivity does not explain additional variance
```

##IHC and FHC: IHC does not explain additional variance 
```{r}
wcn.us.within.plus2 <- glmer(Correct ~ ihc.c + fhc.c + count_range + age.c + (1|SID), 
                            family = "binomial", data = wcn.us.within)
anova(wcn.us.within.plus2, wcn.us.within.plus1, test = 'LRT') ##IHC does not explain additional variance
```
***
...
##Model comparison: Removing effect of Language from FHC model: FHC still significant predictor (*p* = .04)
