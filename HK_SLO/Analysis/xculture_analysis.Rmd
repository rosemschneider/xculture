...
###Analysis: Are Productive counters significantly more likely within language?
```{r}
##HK/SLO/US
...
class.data <- all.data %>%
  filter(Language == "English (US)" |
           Language == "Cantonese" | 
           Language == "Slovenian")%>%
  distinct(SID, Age, Productive, Language)%>%
  mutate(Productive = factor(Productive))
  

prod.lang <- glm(Productive ~ Language + Age, family = "binomial", data = class.data)
summary(prod.lang)
...
prod.lang.ind.eng <- glm(Productive ~ Language + Age, family = "binomial", data = class.data)
...
prod.lang.us.eng <- glm(Productive ~ Language + Age, family = "binomial", data = class.data)
...
xdens.hk <- axis_canvas(pmain.hk, axis = "x") + 
  ...
  geom_density(data = hk_initial, aes(x = IHC, y = ..count.., fill = Productive), 
               alpha=.7, adjust = .5) + 
...
ydens.hk <- axis_canvas(pmain.hk, axis = "y", coord_flip = TRUE) + 
  ...
  geom_density(data = hk_initial, aes(x = FHC, y = ..count.., fill = Productive), 
               alpha=.7, adjust = .5) +
  coord_flip() +
  scale_fill_brewer(palette = "Paired") 

p5 <- insert_xaxis_grob(pmain.hk, xdens.hk, grid::unit(.2, "null"), position = "top")
p6 <- insert_yaxis_grob(p5, ydens.hk, grid::unit(.2, "null"), position = "right")
ggdraw(p6)
...
xdens.slo <- axis_canvas(pmain.slo, axis = "x") + 
  ...
  #                                       fill = Productive), 
  #                stat = 'xdensity', alpha = .5, size = .2, trim = FALSE) + 
  geom_density(data = slo_initial, aes(x = IHC, y = ..count.., fill = Productive), 
               alpha=.7, adjust = .5) + 
  scale_fill_brewer(palette = "Paired") 
...
ydens.slo <- axis_canvas(pmain.slo, axis = "y", coord_flip = TRUE) + 
  ...
  #                                       fill = Productive), 
  #                stat = 'ydensity', alpha = .5, size = .2, trim = FALSE) + 
  geom_density(data = slo_initial, aes(x = FHC, y = ..count.., fill = Productive), 
               alpha=.7, adjust = .5) +
  coord_flip() +
  scale_fill_brewer(palette = "Paired") 

p5 <- insert_xaxis_grob(pmain.slo, xdens.slo, grid::unit(.2, "null"), position = "top")
p6 <- insert_yaxis_grob(p5, ydens.slo, grid::unit(.2, "null"), position = "right")
ggdraw(p6)
...
xdens.hindi <- axis_canvas(pmain.hindi, axis = "x") + 
  ...
  #                                       fill = Productive), 
  #                stat = 'xdensity', alpha = .5, size = .2, trim = FALSE) + 
  geom_density(data = hindi_initial, aes(x = IHC, y = ..count.., fill = Productive), 
               alpha=.7, adjust = .5) + 
  scale_fill_brewer(palette = "Paired") 
...
ydens.hindi <- axis_canvas(pmain.hindi, axis = "y", coord_flip = TRUE) + 
  ...
  #                                       fill = Productive), 
  #                stat = 'ydensity', alpha = .5, size = .2, trim = FALSE) + 
  geom_density(data = hindi_initial, aes(x = FHC, y = ..count.., fill = Productive), 
               alpha=.7, adjust = .5) +
  coord_flip() +
  scale_fill_brewer(palette = "Paired") 

p5 <- insert_xaxis_grob(pmain.hindi, xdens.hindi, grid::unit(.2, "null"), position = "top")
p6 <- insert_yaxis_grob(p5, ydens.hindi, grid::unit(.2, "null"), position = "right")
ggdraw(p6)
...
p6 <- insert_yaxis_grob(p5, ydens.gujarati, grid::unit(.2, "null"), position = "right")
ggdraw(p6)
...
p6 <- insert_yaxis_grob(p5, ydens.ind.eng, grid::unit(.2, "null"), position = "right")
ggdraw(p6)
...
###Correlation between highest count and mean Next Number Performance
####HK/SLO/US
```{r}
###Correlation between HC and mean_unit
ms <- all.data %>%
  filter(Task == "NN")%>%
  group_by(SID, Age, Language)%>%
  summarise(mean_nn = mean(Correct, na.rm = TRUE))
  
ms %<>% as.data.frame(ms) %>%
  dplyr::select(-Language)

hc.nn <- full_join(unique.hc.data, ms, by = "SID")
...
hcn.. %>%
  filter(Language == "Cantonese" | 
           ...
           Language == "Slovenian")%>%
  mutate(Language = factor(Language, levels = c("Cantonese", "Slovenian", "English (US)")))%>%
ggplot(aes(x = highest_count, y = mean_nn, color = Language, group = Language)) + 
  geom_count(stat = "sum", position = position_jitter(),
             show.legend = FALSE) + 
  geom_smooth(aes(fill = Language),  alpha = .2) + 
  theme_bw(base_size = 13) + 
  theme(legend.position = "bottom", 
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
...
###Relationship between age, IHC, and mean NN
####HK/SLO/US
```{r}
##mean performance by language 
ms <- all.data %>%
  filter(Task == "NN")%>%
  group_by(SID, Language, IHC, FHC, prod.gradient, Age)%>%
  summarise(mean = mean(Correct, na.rm = TRUE))

ms %>%
  filter(Language == "Cantonese")%>%
  mutate(IHC.bin = ifelse(IHC < 50, "sub-50", 
                          ifelse((IHC >50 & IHC < 100), "50-100", "100+")), 
         prod.gradient.bin = ifelse(prod.gradient < .33, "0-.33", 
                                    ifelse((prod.gradient > .33 & prod.gradient < .66), ".33-.66", "> .66")))%>%
  mutate(prod.gradient.bin = factor(prod.gradient.bin, levels = c("0-.33", ".33-.66", "> .66")))%>%
  ggplot(aes(x = Age, y = mean, color = prod.gradient.bin, group = prod.gradient.bin)) + 
...
tab_model(sf.slo.within.plus3.ihc)
