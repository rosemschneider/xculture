...
##Mean NN performance - for post-hoc analysis
```{r}
ms.nn <- all.data %>%
  filter(Task == "NN")%>%
  group_by(SID)%>%
...
all.data <- full_join(all.data, ms.nn, by = "SID")
```
...
##Are there significantly more productive counters by language?
```{r}
##HK/SLO/US
...
##How many pariticipants used all 14 prompts?
```{r}
hc.df %>%
...
#Do IHCs differ by language? - Gaussian Mixture Modeling
...
initial_final %>%
  ...
  mutate(Dataset = ifelse(Language == "Cantonese" |
         ...
         Language = factor(Language, levels = c("Cantonese", "English (US)", "Slovenian", "Hindi", "Gujarati", "English (India)")))%>%
...
mtable.sf.hk <- mtable('Base Model' = sf.hk.within.base,
            ...
            'EXP: Mean NN' = sf.hk.within.model.nn,
...
#productivity
prod.model <- summary(sf.hk.within.model1)
AIC.prod.hk <- as.numeric(prod.model$AICtab[1])
chi.sq.prod.hk <- anova(sf.hk.within.base, sf.hk.within.model1, test = 'LRT')
prod.model.df <- data.frame(coef(prod.model)[,0:4])
prod.model.df <- add_rownames(prod.model.df, "Parameter")
prod.model.df %<>%
  mutate(Model = "Productivity")%>%
  mutate(Parameter = ifelse(Parameter == "ProductiveProductive", "Prod. Predictor", as.character(Parameter)))

#FHC
fhc.model <- summary(sf.hk.within.model2)
AIC.fhc.hk <- as.numeric(fhc.model$AICtab[1])
fhc.model.df <- data.frame(coef(fhc.model)[,0:4])
fhc.model.df <- add_rownames(fhc.model.df, "Parameter")
fhc.model.df %<>%
  mutate(Model = "FHC")%>%
  mutate(Parameter = ifelse(Parameter == "fhc.c", "Prod. Predictor", as.character(Parameter)))

#IHC
ihc.model <- summary(sf.hk.within.model3)
AIC.ihc.hk <- as.numeric(ihc.model$AICtab[1])
ihc.model.df <- data.frame(coef(ihc.model)[,0:4])
ihc.model.df <- add_rownames(ihc.model.df, "Parameter")
ihc.model.df %<>%
  mutate(Model = "IHC")%>%
  mutate(Parameter = ifelse(Parameter == "ihc.c", "Prod. Predictor", as.character(Parameter)))

#HCNN
hcnn.model <- summary(sf.hk.within.model4)
AIC.hcnn.hk <- as.numeric(hcnn.model$AICtab[1])
hcnn.model.df <- data.frame(coef(hcnn.model)[,0:4])
hcnn.model.df <- add_rownames(hcnn.model.df, "Parameter")
hcnn.model.df %<>%
  mutate(Model = "HCNN")%>%
  mutate(Parameter = ifelse(Parameter == "highest_contig.c", "Prod. Predictor", as.character(Parameter)))

#join togther 
all.models.hk.unit <- bind_rows(prod.model.df, fhc.model.df, ihc.model.df, hcnn.model.df) 
...
mtable.sf.slo <- mtable('Base Model' = sf.slo.within.base,
            ...
            'EXP: Mean NN' = sf.slo.within.model.nn,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.slo
```

##Model Comparisons
###Base v. Productivity - sig, Chis(1) = 7.18, p = .007, AIC = 1428.9
```{r}
anova(sf.slo.within.base, sf.slo.within.model1, test = 'LRT')
```

 
###Base v. FHC - Sig, Chisq(1) = 12.264, p = .0005, AIC = 1423.8
```{r}
...
###Base v. Highest Contiguous NN - Sig, Chisq(1) = 22.44, p < .0001, AIC = 1413.6
```{r}
...
mtable.sf.us <- mtable('Base Model' = sf.us.within.base,
            ...
            'Exp. model: Mean nn' = sf.us.within.model.nn,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.us
```

##Model comparisons
###Base v. Productivity - NS, Chisq(1) = 1.10, p = .29
```{r}
anova(sf.us.within.base, sf.us.within.model1, test = 'LRT')
```

###Base v. FHC - Sig, Chisq(1) = 18.68, p < .0001, AIC = 1498.8
```{r}
anova(sf.us.within.base, sf.us.within.model2, test = 'LRT')
```

###Base v. IHC - Sig, Chisq(1) = 49.80, p < .0001, AIC = 1467.7
```{r}
anova(sf.us.within.base, sf.us.within.model3, test = 'LRT')
````

###Base v. Highest Contig. - Sig, Chisq(1) = 34.88, p < .0001, AIC = 1482.6
```{r}
...
#post-hoc - mean nn
sf.hindi.within.model.nn <- glmer(Correct ~ mean.nn + count_range + starting_num.c + age.c +
                                      (1|SID), family = "binomial", data = sf.hindi.within)
```
...
mtable.sf.hindi <- mtable('Base Model' = sf.hindi.within.base,
            ...
            'Exp: Mean NN' = sf.hindi.within.model.nn,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.hindi
```

##Model comparisons
###Base v. Productivity - marginal
```{r}
...
anova(sf.hindi.within.base, sf.hindi.within.model.nn, test = 'LRT')
```
...
mtable.sf.gujarati <- mtable('Base Model' = sf.gujarati.within.base,
            ...
            'Exp.: Mean nn' = sf.gujarati.within.model.nn,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.gujarati
```
...
###Base v. FHC - Sig, Chisq(1) = 34.75, p < .0001, AIC = 1172.8
````{r}
anova(wcn.hk.within.base, wcn.hk.within.model2, test = 'LRT')
````

###Base v. IHC = Sig, Chisq(1) = 69.17, p < .0001, AIC = 1138.4
```{r}
...
##Model comparisons
###Base v. Productivity - Sig, Chisq(1) = 20.48, p < .0001, AIC = 860.46
```{r}
anova(wcn.slo.within.base, wcn.slo.within.model1, test = 'LRT')
```

###Base v. FHC - Sig, Chisq(1) = 41.86, p < .0001, AIC = 839.1
```{r}
anova(wcn.slo.within.base, wcn.slo.within.model2, test = 'LRT')
```

###Base v. IHC - Sig, Chisq(1) = 15.13 p = .0001, AIC = 865.81
```{r}
anova(wcn.slo.within.base, wcn.slo.within.model3, test = 'LRT')
```

###Productivity gradient - Sig, Chisq(1) = 27.24, p < .0001, AIC = 838.46
```{r}
...
##Model comparisons
###Base v. Productivity - Sig, Chisq(1) = 14.8, p = .0001, AIC = 1043.2
```{r}
anova(wcn.us.within.base, wcn.us.within.model1, test = 'LRT')
```

###Base v. FHC - Sig, Chisq(1) = 44.1, p < .0001, AIC = 1013.9
```{r}
anova(wcn.us.within.base, wcn.us.within.model2, test = 'LRT')
```

###Base v. IHC - Sig, Chisq(1) = 30.403, p < .0001, AIC = 1010.6
```{r}
...
###Base v. FHC - Sig, Chisq(1) = 21.48, p < .0001, AIC = 681.26
```{r}
anova(wcn.ind.eng.within.base, wcn.ind.eng.within.model2, test = 'LRT')
```

###Base v. IHC - Sig, Chisq(1) = 19.32, p < .0001, AIC = 683.06
```{r}
...
##Model comparisons
###Base v. Productivity - sig, Chisq(1) = 4.64, p = .03, AIC = 859.39
```{r}
anova(wcn.hindi.within.base, wcn.hindi.within.model1, test = 'LRT')
```

###Base v. FHC - sig, Chisq(1) = 27.76, p < .0001, AIC = 830.27
```{r}
anova(wcn.hindi.within.base, wcn.hindi.within.model2, test = 'LRT')
```

###Base v. IHC - Sig, Chisq(1) = 33.88, p < .0001, AIC = 824.15
```{r}
anova(wcn.hindi.within.base, wcn.hindi.within.model3, test = 'LRT')
```

###Base v. Prod. gradient - Sig, Chisq(1) = 33.542, p < .0001, AIC = 806.07
```{r}
...
##Model comparisons
###Base v. Productivity - Sig, Chisq(1) = 10.2, p = .002, AIC = 788.92
```{r}
anova(wcn.gujarati.within.base, wcn.gujarati.within.model1, test = 'LRT')
```

###Base v. FHC - Sig, Chisq(1) = 36.43, p < .0001, AIC  762.64
```{r}
anova(wcn.gujarati.within.base, wcn.gujarati.within.model2, test = 'LRT')
```

###Base v. IHC = Sig, Chisq(1) = 33.46, p < .0001, AIC = 765.61
```{r}
...
mtable.sf.cross <- mtable('Base Model' = sf.cross.base,
            ...
            'Exp: NN' = sf.cross.model.nn,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.cross
```

##Model Comparisons
###Base v. productivity - NS, p = .32
```{r}
...
###Base v. Highest contig. - Sig, Chisq(1) = 25.502, p < .0001, AIC = 4363.6
```{r}
...
lang.prod.int.hk.noint <- glmer(Correct ~ highest_contig.c + Language*ihc.c + count_range + starting_num.c + age.c + wppsi.c + 
                            ...
                            control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
lang.prod.int.hk <- glmer(Correct ~ highest_contig.c*Language + Language*ihc.c + count_range + starting_num.c + age.c + wppsi.c + 
                            (1|SID), family = "binomial", data = sf.df.cross, 
                          control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
...
mtable.sf.cross.india <- mtable('Base Model' = sf.cross.base.india,
            ...
            'Exp: Mean NN' = sf.cross.model.nn.india,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
...
###Base v. Highest contig. - Sig, Chisq(1) = 12.49, p = .0004, AIC = 3948.2
```{r}
anova(sf.cross.base.india, sf.cross.model3.india, test = 'LRT')
```

#Exploratory - Prod. gradient, mean NN
```{r}
...
#add mean nn, compare
sf.cross.plus.nn.india <- glmer(Correct ~ mean.nn + highest_contig.c + Language*ihc.c + count_range + starting_num.c + age.c + wppsi.c +
                           (1|SID), 
                       family = "binomial", data = sf.df.cross.india, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))

anova(sf.cross.model3.india, sf.cross.plus.nn.india, test = 'LRT')
...
sf.india.lang.noint <- glmer(Correct ~ highest_contig.c + Language*ihc.c + count_range + starting_num.c + age.c + wppsi.c + (1|SID), 
                         ...
                         control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
sf.india.lang.int <- glmer(Correct ~ highest_contig.c*Language + Language*ihc.c + count_range + starting_num.c + age.c + wppsi.c + (1|SID), 
                         data = tmp, family = "binomial", 
                         control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
...
##Model comparisons
###Base v. Productivity - Sig, Chisq(1) = 10.1, p = .001, AIC = 2555.4
```{r}
anova(wcn.cross.base.india, wcn.cross.model1.india, test = 'LRT')
```

###Base v. FHC - Sig, Chisq(1) = 22.994, p < .0001, AIC = 2542.5
```{r}
anova(wcn.cross.base.india, wcn.cross.model2.india, test = 'LRT')
```

###Base v. Prod. gradient - Sig, Chisq(1) = 16.23, p < .0001, AIC = 2549.3
```{r}
...
lang.prod.int.india.noint <- glmer(Correct ~ fhc.c + Language*ihc.c + count_range + starting_num.c + age.c + wppsi.c + 
                            ...
                            control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
lang.prod.int.india <- glmer(Correct ~ fhc.c*Language + Language*ihc.c + count_range + starting_num.c + age.c + wppsi.c + 
                            (1|SID), family = "binomial", data = wcn.df.cross.india, 
                          control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
...
##Model comparisons
###Base v. Productivity - Sig, Chisq(1) = 14.82, p = .0001, AIC = 2966.5
```{r}
anova(wcn.cross.base, wcn.cross.model1, test = 'LRT')
```

###Base v. FHC - Sig, Chisq(1) = 38.04, p < .0001, AIC = 2943.3
```{r}
...
```{r}
wcn.prod.int <- glmer(Correct ~ Language*fhc.c + Language*ihc.c + count_range + starting_num.c + age.c + wppsi.c + (1|SID), 
                         data = wcn.df.cross, family = "binomial", 
                         control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
