...
###IHC, FHC by language
Tables 3 and 9 (overall IHC and FHC)
```{r}
#IHC, FHC by language
all.data %>%
  filter(!is.na(Language))%>%
...
###IHC, FHC by language and Resilience
Tables 3 and 9 (IHC and FHC by Resilience)
```{r}
...
#finally, what are the distributions of prompts by language? 
error.freq %>%
  filter(IHC != 140)%>%
...
##what are the deltas by IHC stopping type? 
ihc.dist.mean <- ihc.dist %>%
  mutate(delta = FHC-IHC)%>%
  mutate(IHC_stop = factor(IHC_stop, levels = c("Decade end", "Decade beginning", "Mid-decade", 
                              "Hindi/Gujarati pre-decade transition")))

model.test <- lm(delta ~ IHC_stop, data = subset(ihc.dist.mean, Language == "Cantonese"))
summary(model.test) #ns for Cantonese

model.test <- lm(delta ~ IHC_stop, data = subset(ihc.dist.mean, Language == "English (US)"))
summary(model.test) #significant for English (US)

model.test <- lm(delta ~ IHC_stop, data = subset(ihc.dist.mean, Language == "Slovenian"))
summary(model.test) #ns for Slovenian

model.test <- lm(delta ~ IHC_stop, data = subset(ihc.dist.mean, Language == "Hindi"))
summary(model.test) #ns for Hindi

model.test <- lm(delta ~ IHC_stop, data = subset(ihc.dist.mean, Language == "Gujarati"))
summary(model.test) #ns for Gujarati

model.test <- lm(delta ~ IHC_stop, data = subset(ihc.dist.mean, Language == "English (India)"))
summary(model.test) #ns for Indian English
```
...
###SOM Analysis: What proportion of kids in highest proportion cluster (from GMM) are labeled as resilient in each language?
First, set up GMM
```{r}
library(mclust)
...
Now fit GMM on IHC for every language
...
###SOM Analysis: Do Resilient counters have higher HCNNs?
...
###SOM Analysis: Do transparent languages allow nonproductive counters to generate higher HCNNs?
...
##### TEST WITH RANDOM EFFECT OF STARTING NUMBER ####
#base
sf.hk.within.base.t <- glmer(Correct ~ count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.hk.within)
#Productivity
sf.hk.within.model1.t <- glmer(Correct ~ Productive + count_range +  age.c + 
                               (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.hk.within)
#FHC
sf.hk.within.model2.t <- glmer(Correct ~ fhc.c + count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.hk.within)
#IHC
sf.hk.within.model3.t <- glmer(Correct ~ ihc.c + count_range + age.c + (1|SID) + (1|starting_num.c), 
                             family = "binomial", 
                           data = sf.hk.within)

#Highest contig.
sf.hk.within.model4.t <- glmer(Correct ~ highest_contig.c + count_range  + age.c + (1|SID) + (1|starting_num.c), 
                             family = "binomial", 
                           data = sf.hk.within)


tab_model(sf.hk.within.model3.t)
tab_model(sf.hk.within.model3)
...
##test
mtable.sf.hk <- mtable('Base Model' = sf.hk.within.base.t,
            'Model 1: Productivity' = sf.hk.within.model1.t,
            'Model 2: FHC' = sf.hk.within.model2.t,
            'Model 3: IHC' = sf.hk.within.model3.t,
            'Model 4: Highest Contig.' = sf.hk.within.model4.t,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.hk
...
#####TEST with RANDOM EFFECT ####
#Base = IHC
sf.hk.within.plus1 <- glmer(Correct ~ ihc.c + count_range + age.c + (1|SID) + (1|starting_num.c), 
                             family = "binomial", 
                           data = sf.hk.within)

#IHC and Highest Contiguous
sf.hk.within.plus2.hc <- glmer(Correct ~ highest_contig.c + ihc.c+ count_range + age.c + (1|SID) + (1|starting_num.c), 
                            family = "binomial", data = sf.hk.within)
...
#IHC and FHC
sf.hk.within.plus2.fhc <- glmer(Correct ~ fhc.c + ihc.c + count_range  + age.c + (1|SID) + (1|starting_num.c), 
                            family = "binomial", data = sf.hk.within)
...
sf.hk.within.plus3 <- glmer(Correct ~ highest_contig.c + fhc.c + ihc.c + count_range + age.c + 
                              (1|SID) + (1|starting_num.c), family = "binomial", data = sf.hk.within)
...
####TEST WITH RANDOM EFFECT ####
#base
sf.slo.within.base.t <- glmer(Correct ~ count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.slo.within)
#productivity
sf.slo.within.model1.t <- glmer(Correct ~ Productive + count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.slo.within)
#FHC
sf.slo.within.model2.t <- glmer(Correct ~ fhc.c + count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.slo.within)
#IHC
sf.slo.within.model3.t <- glmer(Correct ~ ihc.c + count_range +  age.c + (1|SID) + (1|starting_num.c), 
                             family = "binomial", 
                           data = sf.slo.within)
#highest contiguous NN
sf.slo.within.model4.t <- glmer(Correct ~ highest_contig.c + count_range + age.c + (1|SID) + (1|starting_num.c), 
                             family = "binomial", 
                           data = sf.slo.within) 

tab_model(sf.slo.within.model4.t)
tab_model(sf.slo.within.model4)
...
mtable.sf.slo <- mtable('Base Model' = sf.slo.within.base.t,
            'Model 1: Productivity' = sf.slo.within.model1.t,
            'Model 2: FHC' = sf.slo.within.model2.t,
            'Model 3: IHC' = sf.slo.within.model3.t,
            'Model 4: Highest Contig.' = sf.slo.within.model4.t,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.slo
...
anova(sf.slo.within.base.t, sf.slo.within.model1.t, test = 'LRT')
```
...
###TESTING WITH RANDOM EFFECT ####

#Base - HCNN
sf.slo.within.plus1 <- glmer(Correct ~ highest_contig.c + count_range + age.c + (1|SID) + (1|starting_num.c), 
                             data = sf.slo.within, family = "binomial")

#Now add second-lowest AIC
#HCNN + FHC
sf.slo.within.plus2.fhc <- glmer(Correct ~ fhc.c + highest_contig.c + count_range + age.c + (1|SID) + (1|starting_num.c), 
                             data = sf.slo.within, family = "binomial")
...
##Now add Productivity
#Productive + FHC + HCNN
sf.slo.within.plus3 <- glmer(Correct ~ Productive + fhc.c + highest_contig.c + count_range + age.c + 
                               (1|SID) + (1|starting_num.c), family = "binomial", data = sf.slo.within)
#Compare
anova(sf.slo.within.plus2.fhc, sf.slo.within.plus3, test = 'LRT')#NS

##FHC and HCNN best predictors, Productivity does not add anything. Now add IHC
#IHC + FHC + HCNN
sf.slo.within.plus3.ihc <- glmer(Correct ~ ihc.c + fhc.c + highest_contig.c + count_range + age.c + (1|SID) + (1|starting_num.c), 
                             data = sf.slo.within, family = "binomial")
...
#base
sf.us.within.base.t <- glmer(Correct ~ count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.us.within)
#productivity
sf.us.within.model1.t <- glmer(Correct ~ Productive + count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.us.within)
#FHC
sf.us.within.model2.t <- glmer(Correct ~ fhc.c + count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.us.within)
#IHC
sf.us.within.model3.t <- glmer(Correct ~ ihc.c + count_range +  age.c + (1|SID) + (1|starting_num.c), 
                             family = "binomial", 
                           data = sf.us.within, control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#Highest contiguous
sf.us.within.model4.t <- glmer(Correct ~ highest_contig.c + count_range + age.c + (1|SID) + (1|starting_num.c), 
                             family = "binomial", 
                           data = sf.us.within) 
...
mtable.sf.us <- mtable('Base Model' = sf.us.within.base.t,
            'Model 1: Productivity' = sf.us.within.model1.t,
            'Model 2: FHC' = sf.us.within.model2.t,
            'Model 3: IHC' = sf.us.within.model3.t,
            'Model 4: Highest Contig.' = sf.us.within.model4.t,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.us
...
####Testing with random effect #####
IHC
sf.us.within.plus1 <- glmer(Correct ~ ihc.c + count_range  + age.c + (1|SID) + (1|starting_num.c), 
                             data = sf.us.within, family = "binomial", 
                            control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))

#IHC + HCNN
sf.us.within.plus2.highest.contig <- glmer(Correct ~ highest_contig.c + ihc.c + count_range +  age.c + (1|SID) + (1|starting_num.c), 
                             data = sf.us.within, family = "binomial", 
                             control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
...
#IHC + FHC + HCNN
sf.us.within.plus3 <- glmer(Correct ~ fhc.c + highest_contig.c + ihc.c + count_range + age.c + (1|SID) + (1|starting_num.c), 
                             data = sf.us.within, family = "binomial", 
                            control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#compare 3, minus fhc
...
#### TESTING WITH RANDOM EFFECT ####
#Base
sf.hindi.within.base.t <- glmer(Correct ~ count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.hindi.within)
#Productivity
sf.hindi.within.model1.t <- glmer(Correct ~ Productive + count_range + age.c  + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.hindi.within)
#FHC
sf.hindi.within.model2.t <- glmer(Correct ~ fhc.c + count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.hindi.within)
#IHC
sf.hindi.within.model3.t <- glmer(Correct ~ ihc.c + count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.hindi.within)
#Highest contig
sf.hindi.within.model4.t <- glmer(Correct ~ highest_contig.c + count_range  + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.hindi.within)
...
mtable.sf.hindi <- mtable('Base Model' = sf.hindi.within.base.t,
            'Model 1: Productivity' = sf.hindi.within.model1.t,
            'Model 2: FHC' = sf.hindi.within.model2.t,
            'Model 3: IHC' = sf.hindi.within.model3.t,
            'Model 4: Highest Contig.' = sf.hindi.within.model4.t,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.hindi
...
###TESTING WITH RANDOM EFFECT ####
##NB THAT HCNN IS NOW STRONGER THAN IHC. IHC STILL SIG.
#IHC
sf.hindi.within.plus1 <- glmer(Correct ~ highest_contig.c + count_range + age.c  + (1|SID) + (1|starting_num.c), 
                             family = "binomial", 
                           data = sf.hindi.within)
#HCNN + IHC 
sf.hindi.within.plus2.hc <- glmer(Correct ~ ihc.c + highest_contig.c + count_range + age.c +  (1|SID) + (1|starting_num.c), 
                            family = "binomial", data = sf.hindi.within)

##compare
anova(sf.hindi.within.plus1, sf.hindi.within.plus2.hc, test = 'LRT') #ihc significantly adds to the model

#HCNN + FHC + IHC
sf.hindi.within.plus3 <- glmer(Correct ~ fhc.c + ihc.c + highest_contig.c + count_range + age.c + (1|SID) + (1|starting_num.c), 
                            family = "binomial", data = sf.hindi.within)
#Comparison of 3 minus fhc
anova(sf.hindi.within.plus1, sf.hindi.within.plus2.hc, sf.hindi.within.plus3, test = 'LRT') #FHC does not
...
####TESTING WITH RANDOM EFFECT ###
#Base
sf.gujarati.within.base.t <- glmer(Correct ~ count_range + age.c +  (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.gujarati.within)
#Productivity
sf.gujarati.within.model1.t <- glmer(Correct ~ Productive + count_range  + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.gujarati.within)
#FHC
sf.gujarati.within.model2.t <- glmer(Correct ~ fhc.c + count_range + age.c +  (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.gujarati.within)
#IHC
sf.gujarati.within.model3.t <- glmer(Correct ~ ihc.c + count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.gujarati.within)
#Highest contig.
sf.gujarati.within.model4.t <- glmer(Correct ~ highest_contig.c + count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                           data = sf.gujarati.within)
...
mtable.sf.gujarati <- mtable('Base Model' = sf.gujarati.within.base.t,
            'Model 1: Productivity' = sf.gujarati.within.model1.t,
            'Model 2: FHC' = sf.gujarati.within.model2.t,
            'Model 3: IHC' = sf.gujarati.within.model3.t,
            'Model 4: Highest Contig.' = sf.gujarati.within.model4.t,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.gujarati
...
anova(sf.gujarati.within.base.t, sf.gujarati.within.model1.t, test = 'LRT')
```

####Base v. FHC - NS, p = .3
```{r}
anova(sf.gujarati.within.base, sf.gujarati.within.model2, test = 'LRT')
anova(sf.gujarati.within.base.t, sf.gujarati.within.model2.t, test = 'LRT')
```

####Base v. IHC - p = .19
```{r}
anova(sf.gujarati.within.base, sf.gujarati.within.model3, test = 'LRT')
anova(sf.gujarati.within.base.t, sf.gujarati.within.model3.t, test = 'LRT')
```

####Base v. Highest contig, NS p = .2
```{r}
anova(sf.gujarati.within.base, sf.gujarati.within.model4, test = 'LRT')
anova(sf.gujarati.within.base.t, sf.gujarati.within.model4.t, test = 'LRT')
```
...
#base
wcn.hk.within.base.t <- glmer(Correct ~ count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                            data = wcn.hk.within)
#productivity
wcn.hk.within.model1.t <- glmer(Correct ~ Productive + count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                            data = wcn.hk.within)
#FHC
wcn.hk.within.model2.t <- glmer(Correct ~ fhc.c + count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                            data = wcn.hk.within)
#IHC
wcn.hk.within.model3.t <- glmer(Correct ~ ihc.c + count_range + age.c + (1|SID) + (1|starting_num.c), family = "binomial", 
                            data = wcn.hk.within)
...
mtable.wcn.hk <- mtable('Base Model' = wcn.hk.within.base.t,
            'Model 1: Productivity' = wcn.hk.within.model1.t,
            'Model 2: FHC' = wcn.hk.within.model2.t,
            'Model 3: IHC' = wcn.hk.within.model3.t,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.wcn.hk
...
tab_model(wcn.hk.within.model3)
tab_model(wcn.hk.within.model3.t)
```
...
###Productivity gradient - Sig, Chisq(1) = 43.52, p < .0001, AIC = 860.04
```{r}
...
###Base v. FHC - Sig, Chisq(1) = 44.93, p < .0001, AIC = 1012.6
```{r}
...
####FOLLOW UP: Do Hindi and Gujarati speakers have a bias for picking 1st or 2nd item? 
```{r}
bias.df <- all.data %>%
  filter(Task == "SF")%>%
  mutate(Task_item = as.numeric(as.character(Task_item)),
         Response = as.numeric(as.character(Response)),
         choice.order = ifelse(((Trial_number == "1" | Trial_number == "3" | 
                                  Trial_number == "5" | Trial_number == "7" |
                                  Trial_number == "9" | Trial_number == "11") & Response == Task_item + 1), "Second", 
                               ifelse(((Trial_number == "1" | Trial_number == "3" | 
                                  Trial_number == "5" | Trial_number == "7" |
                                  Trial_number == "9" | Trial_number == "11") & Response == Task_item + 2), "First", 
                               ifelse(((Trial_number == "2" | Trial_number == "4" | 
                                  Trial_number == "6" | Trial_number == "8" |
                                  Trial_number == "10" | Trial_number == "12") & Response == Task_item + 1), "First", 
                               ifelse(((Trial_number == "2" | Trial_number == "4" | 
                                  Trial_number == "6" | Trial_number == "8" |
                                  Trial_number == "10" | Trial_number == "12") & Response == Task_item + 2), "Second", NA)))))

##how frequently do these kids select each option? 
bias.df %>%
  filter(!is.na(choice.order), 
         Language == "Hindi" |
           Language == "Gujarati")%>%
  mutate(Task_item = factor(Task_item, levels = c("5", "7", "16", 
                                                  "24", "52", "71", 
                                                  ...
                                                  "116", "224", "252", 
                                                  "271")))%>%
  group_by(Language, Task_item, choice.order)%>%
  summarise(n = n()) %>%
  group_by(Language)%>%
  mutate(prop = n/sum(n))%>%
  ggplot(aes(x = Task_item, y= prop, fill = choice.order)) + 
  geom_bar(stat='identity', position = position_dodge()) + 
  facet_grid(~Language)

#within language, are they biased to pick the first alternative more than chance? 
bias.df %<>%
  mutate(choice.first = ifelse(choice.order == "First", 1, 0), 
         choice.second = ifelse(choice.order == "Second",1, 0))

hindi.bias <- matrix(c(sum(subset(bias.df, Language == "Hindi")$choice.first, na.rm =TRUE), 
              sum(subset(bias.df, Language == "Hindi")$choice.second, na.rm = TRUE)))
chisq.test(hindi.bias)

gujarati.bias <- matrix(c(sum(subset(bias.df, Language == "Gujarati")$choice.first, na.rm =TRUE), 
              sum(subset(bias.df, Language == "Gujarati")$choice.second, na.rm = TRUE)))
chisq.test(gujarati.bias)

hk.bias <- matrix(c(sum(subset(bias.df, Language == "Cantonese")$choice.first, na.rm =TRUE), 
              sum(subset(bias.df, Language == "Cantonese")$choice.second, na.rm = TRUE)))
chisq.test(hk.bias)

slo.bias <- matrix(c(sum(subset(bias.df, Language == "Slovenian")$choice.first, na.rm =TRUE), 
              sum(subset(bias.df, Language == "Slovenian")$choice.second, na.rm = TRUE)))
chisq.test(slo.bias)


###Re-run, filter out large numbers
bias.df.small <- bias.df %>%
  filter(Task_item < 100)

hindi.bias <- matrix(c(sum(subset(bias.df.small, Language == "Hindi")$choice.first, na.rm =TRUE), 
              sum(subset(bias.df.small, Language == "Hindi")$choice.second, na.rm = TRUE)))
chisq.test(hindi.bias)

gujarati.bias <- matrix(c(sum(subset(bias.df.small, Language == "Gujarati")$choice.first, na.rm =TRUE), 
              sum(subset(bias.df.small, Language == "Gujarati")$choice.second, na.rm = TRUE)))
chisq.test(gujarati.bias)


####If we remove the large numbers, what are the descriptives? 
bias.df %>%
  filter(Task_item < 200)%>%
  group_by(Language)%>%
  summarise(mean = mean(Correct, na.rm = TRUE), 
            sd = sd(Correct, na.rm = TRUE))

#do children perform below chance for smaller ns? 
lang.small.ms <- bias.df.small %>%
  group_by(SID, Language)%>%
  summarise(mean = mean(Correct, na.rm = TRUE))

t.test(subset(lang.small.ms, Language == "Hindi")$mean, mu = .5, var.equal = TRUE)

t.test(subset(lang.small.ms, Language == "Gujarati")$mean, mu = .5, var.equal = TRUE)

#finally, what if we filter out subset-knowers?
bias.df %>%
  filter(Knower.level == "CP-knower")%>%
  group_by(Language)%>%
  summarise(mean = mean(Correct, na.rm = TRUE), 
            sd = sd(Correct, na.rm = TRUE))
```
...
us.cheung <- all.data %>%
  ...
  mutate(counting.classification = ifelse(IHC < 20, "Low", 
         ...
         counting.classification = factor(counting.classification, levels = c("Low", "Medium", "High", "Very High")),
         number.magnitude = ifelse((Task_item == "5" | Task_item == "7"), "Small", 
                                   ifelse(Task_item == "16", "Medium", 
                                          ifelse(Task_item == "24", "Large", 
                                                 ifelse((Task_item == "52" | Task_item == "71"), "Very Large", "Super-large")))), 
         number.magnitude = factor(number.magnitude, levels = c("Small", "Medium", "Large", "Very Large", "Super-large")))

#cheung style accuracy
us.cheung %>%
  group_by(SID, counting.classification, number.magnitude)%>%
  summarise(mean = mean(Correct, na.rm = TRUE))%>%
  mutate(mean.factor = cut(mean, breaks = c(0, .33, .5, .67, .83, 1), include.lowest = TRUE))%>%
  group_by(counting.classification, number.magnitude, mean.factor)%>%
  summarise(n = n())%>%
  group_by(counting.classification)%>%
  mutate(prop = n/sum(n)) %>%
  ggplot(aes(x = mean.factor, y= prop, fill = number.magnitude)) + 
  geom_bar(stat = "identity", position = position_dodge()) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  facet_wrap(~counting.classification, ncol = 2)
...
#base
sf.cross.base.t <- glmer(Correct ~ Language*ihc.c + count_range + age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#productivity
sf.cross.model1.t <- glmer(Correct ~ Productive + Language*ihc.c + count_range  +age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#fhc
sf.cross.model2.t <- glmer(Correct ~ fhc.c + Language*ihc.c + count_range  + age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#highest contig.
sf.cross.model3.t <- glmer(Correct ~ highest_contig.c + Language*ihc.c + count_range  + age.c + wppsi.c +
...
mtable.sf.cross <- mtable('Base Model' = sf.cross.base.t,
            'Model 1: Productivity' = sf.cross.model1.t,
            'Model 2: FHC' = sf.cross.model2.t,
            'Model 3: Highest Contig.' = sf.cross.model3.t, 
            # 'Exp: Prod. gradient' = sf.cross.model.gain,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.cross
...
tab_model(sf.cross.model3.t, transform = NULL)
```
...
#base
sf.cross.base.slo.t <- glmer(Correct ~ Language*ihc.c + count_range + age.c + wppsi.c + (1|SID) + 
                               (1|starting_num.c), 
                       family = "binomial", data = slo.ref, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
...
#model output
tab_model(sf.cross.base.slo.t, transform = NULL)
#productivity
sf.cross.model1.slo.t <- glmer(Correct ~ Productive + Language*ihc.c + count_range +age.c + wppsi.c + (1|SID) + 
                               (1|starting_num.c), 
                       family = "binomial", data = slo.ref, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
...
#model output
tab_model(sf.cross.model1.slo.t, transform = NULL)
#fhc
sf.cross.model2.slo.t <- glmer(Correct ~ fhc.c + Language*ihc.c + count_range  + age.c + wppsi.c + (1|SID) + 
                               (1|starting_num.c), 
                       family = "binomial", data = slo.ref, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
tab_model(sf.cross.model2.slo, transform = NULL)
#highest contig.
sf.cross.model3.slo.t <- glmer(Correct ~ highest_contig.c + Language*ihc.c + count_range + age.c + wppsi.c +
                           (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = slo.ref, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))

tab_model(sf.cross.model3.slo.t, transform = NULL)

```



###Follow up: Testing whether results of cross-linguistic analyses dependent on IHC
```{r}
#base
sf.cross.base.ihc <- glmer(Correct ~ Language + count_range + age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#productivity
sf.cross.model1.ihc <- glmer(Correct ~ Productive + Language + count_range  +age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#fhc
sf.cross.model2.ihc <- glmer(Correct ~ fhc.c + Language + count_range  + age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#highest contig.
sf.cross.model3.ihc <- glmer(Correct ~ highest_contig.c + Language + count_range  + age.c + wppsi.c +
                           (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))

mtable.sf.cross <- mtable('Base Model' = sf.cross.base.ihc,
            'Model 1: Productivity' = sf.cross.model1.ihc,
            'Model 2: FHC' = sf.cross.model2.ihc,
            'Model 3: Highest Contig.' = sf.cross.model3.ihc, 
            # 'Exp: Prod. gradient' = sf.cross.model.gain,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.cross

##comparison
##lowest AIC is HCNN, start with that as base
#add FHC, second strongest predictor
sf.cross.plusFHC <- glmer(Correct ~  fhc.c + highest_contig.c + Language + count_range  + age.c + wppsi.c +
                           (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
...
#now compare
anova(sf.cross.model3.ihc, sf.cross.plusFHC, test = 'LRT') #fhc is significant

#now add resilience
sf.cross.plusprod <- glmer(Correct ~  Productive + fhc.c + highest_contig.c + Language + count_range  + age.c + wppsi.c +
                           (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
...
#now compare 
anova(sf.cross.plusFHC, sf.cross.plusprod, test = 'LRT') # prod is also significant

#summary of final model
summary(sf.cross.plusprod)

####WITH SLO AS REFERENCE GROUP ####

#base
sf.cross.base.ihc.slo <- glmer(Correct ~ Language + count_range + age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = slo.ref, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#productivity
sf.cross.model1.ihc.slo <- glmer(Correct ~ Productive + Language + count_range  +age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = slo.ref, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#fhc
sf.cross.model2.ihc.slo <- glmer(Correct ~ fhc.c + Language + count_range  + age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = slo.ref, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#highest contig.
sf.cross.model3.ihc.slo <- glmer(Correct ~ highest_contig.c + Language + count_range  + age.c + wppsi.c +
                           (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = slo.ref, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))

mtable.sf.cross <- mtable('Base Model' = sf.cross.base.ihc.slo,
            'Model 1: Productivity' = sf.cross.model1.ihc.slo,
            'Model 2: FHC' = sf.cross.model2.ihc.slo,
            'Model 3: Highest Contig.' = sf.cross.model3.ihc.slo, 
            # 'Exp: Prod. gradient' = sf.cross.model.gain,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.cross

#now add resilience
sf.cross.plusprod.slo <- glmer(Correct ~  Productive + fhc.c + highest_contig.c + Language + count_range  + age.c + wppsi.c +
                           (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = slo.ref, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#summary of final model
summary(sf.cross.plusprod.slo)
...
#base
sf.cross.base.india.t <- glmer(Correct ~ Language*ihc.c + count_range + age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross.india, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#productivity
sf.cross.model1.india.t <- glmer(Correct ~ Productive + Language*ihc.c + count_range +age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross.india, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#fhc
sf.cross.model2.india.t <- glmer(Correct ~ fhc.c + Language*ihc.c + count_range  + age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross.india, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#highest contig.
sf.cross.model3.india.t <- glmer(Correct ~ highest_contig.c + Language*ihc.c + count_range  + age.c + wppsi.c +
                           (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross.india, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
...
mtable.sf.cross.india <- mtable('Base Model' = sf.cross.base.india.t,
            'Model 1: Productivity' = sf.cross.model1.india.t,
            'Model 2: FHC' = sf.cross.model2.india.t,
            'Model 3: Highest Contig.' = sf.cross.model3.india.t,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.cross.india
...
###Follow up: Testing whether effects are contingent on IHC
```{r}
#removing IHC

#base
sf.cross.base.india.ihc <- glmer(Correct ~ Language + count_range + age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross.india, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#productivity
sf.cross.model1.india.ihc <- glmer(Correct ~ Productive + Language + count_range +age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross.india, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#fhc
sf.cross.model2.india.ihc <- glmer(Correct ~ fhc.c + Language + count_range  + age.c + wppsi.c + (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross.india, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#highest contig.
sf.cross.model3.india.ihc <- glmer(Correct ~ highest_contig.c + Language + count_range  + age.c + wppsi.c +
                           (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross.india, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))

mtable.sf.cross.india <- mtable('Base Model' = sf.cross.base.india.ihc,
            'Model 1: Productivity' = sf.cross.model1.india.ihc,
            'Model 2: FHC' = sf.cross.model2.india.ihc,
            'Model 3: Highest Contig.' = sf.cross.model3.india.ihc,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.cross.india

#comparisons
##HCNN still lowest AIC, so use that as a base
#add FHC, which has second lowest AIC
sf.cross.india.plusFHC <- glmer(Correct ~ fhc.c + highest_contig.c + Language + count_range  + age.c + wppsi.c +
                           (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross.india, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
...
#compare
anova(sf.cross.model3.india.ihc, sf.cross.india.plusFHC, test = 'LRT') #FHC sig

#now add Resilience
sf.cross.india.plusprod <- glmer(Correct ~ Productive + fhc.c + highest_contig.c + Language + count_range  + age.c + wppsi.c +
                           (1|SID) + (1|starting_num.c), 
                       family = "binomial", data = sf.df.cross.india, 
                        control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
...
#compare
anova(sf.cross.india.plusFHC, sf.cross.india.plusprod, test= 'LRT') # prod marginal

#summary of full model with FHC and HCNN
summary(sf.cross.india.plusFHC)
```
